...
class Utils {
    static TempToolTip(msg, durationMS){
        ToolTip(msg)
        SetTimer(() => ToolTip(), -durationMS)
    }

    ; function to display a tooltip as long as the layer is enabled
    static ShowState(name, stateNode, state){
        ShowMessage(){
    ...
    static SendCharIfNoCombo(character, state){
        isRelease := state = 0
        if not isRelease
            return
        
        isCombo := Utils.CheckIfCombo(character)
        if (not isCombo){    
